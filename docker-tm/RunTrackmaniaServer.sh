...
sed -i "s/<login></<login>${SERVER_LOGIN}</" /opt/tmserver/GameData/Config/dedicated_cfg.txt
sed -i "s/<password></<password>${SERVER_PASSWORD}</" /opt/tmserver/GameData/Config/dedicated_cfg.txt
sed -i "s/<validation_key></<validation_key>${SERVER_VALIDATION_KEY}</" /opt/tmserver/GameData/Config/dedicated_cfg.txt

echo "Server config dedicated_cfg.txt is"
cat /opt/tmserver/GameData/Config/dedicated_cfg.txt

echo "Launching Server"
exec ./TrackmaniaServer /dedicated_cfg=dedicated_cfg.txt /game_settings=MatchSettings/custom_game_settings.txt /nodaemon /internet
